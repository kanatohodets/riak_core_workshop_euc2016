defmodule HandoffKV.OpFSM do
  ...
  def op(op, key, n, w) do
	req_id = req_id()
	HandoffKV.OpFSM.Sup.start_op_fsm([req_id, self(), op, key, n, w])
	{:ok, req_id}
  end
  ...
  def prepare(:timeout, %{n: n, key: key, req_id: req_id, op: op}=state_data) do
    ...
    pref_list = :riak_core_apl.get_apl(doc_idx, n, HandoffKV.Service)
  ...
  def execute(:timeout, %{req_id: req_id, op: op, preflist: pref_list}=state_data) do
    ...
    command = {req_id, op}
    :riak_core_vnode_master.command(pref_list, command, {:fsm, :undefined, self()}, HandoffKV.Vnode_master)
