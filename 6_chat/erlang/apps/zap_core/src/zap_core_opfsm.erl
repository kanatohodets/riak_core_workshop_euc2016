...
op(Op, Key, N, W) ->
    ReqID = req_id(),
    zap_core_opfsm_sup:start_op_fsm([ReqID, self(), Op, Key, N, W]),
    {ok, ReqID}.
...
prepare(timeout, SD0=#state{op=Op, req_id=ReqID, n=N, key=Key}) ->
    ...
    Preflist = riak_core_apl:get_apl(DocIdx, N, zap_core_service),
