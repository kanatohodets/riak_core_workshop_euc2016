...
%% Store some data
handle_command({ReqID, {store, Key, Data}}, _Sender, State=#state{db=Db, partition=Partition}) ->
    lager:info("storing ~p data on vnode ~p", [Key, Partition]),
    Res = ets:insert(Db, {Key, Data}),
    {reply, {ReqID, {Res}}, State};

%% Fetch some data
handle_command({ReqID, {fetch, Key}}, _Sender, State=#state{db=Db, partition=Partition}) ->
    case ets:lookup(Db, Key) of
      [] -> 
            lager:info("vnode ~p was asked for key ~p, but it's not here", [Partition, Key]),
            {reply, {ReqID, {not_found}}, State};
      [{Key, Data}] -> 
            lager:info("vnode ~p fetching ~p", [Partition, Key]),
            {reply, {ReqID, {Key, Data}}, State}
    end.
